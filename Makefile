...
export REPO=renderedtext/za_igranje
IMAGE=$(REPO):$(TAG)
...
image.test: image.build
	docker run --rm --name za_igranje -p 4004:4000 -d $(IMAGE)
	# run some tests...
	docker kill za_igranje

deploy: abort-on-uncommited-changes image.push config-gen
	kubectl apply -f deploy.yml
	kubectl get svc/za-igranje
